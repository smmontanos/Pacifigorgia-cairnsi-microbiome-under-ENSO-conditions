...
qiime tools import --type 'SampleDta[PairedEndSequencesWithQuality]' --input-path ~/manifest.csv --input-format PairedEndFastqManifestPhred33 --output-path ~/demux-paired-end.qza
...
qiime vsearch join-pairs --i-demultiplexed-seqs demux-paired-end.qza --o-joined-sequences demux-joined.qza --p-minmergelen 248 --p-maxmergelen 258  --p-maxdiffs 10

#Quality filter based on q-score

qiime quality-filter q-score-joined --i-demux demux-joined.qza --o-filtered-sequences demux-joined-final-filtered-q20.qza --o-filter-stats demux-joined-filter-stats.qza --p-min-quality 20
...
qiime feature-classifier classify-sklearn --i-classifier silva_132_99_515F_806R_nb_classifier.qza --i-reads rep-seqs-dablur-250final.qza --o-classification taxonomy_silva.qza
...
#To export the ASV table

qiime tools export table-minsamp3-minfrec3.qza --output-dir table-mce-min3

#To convert the ASV table from biom format to txt file

biom convert -i table-mce-min3/feature-table.biom -o table-mce-min3/export-table-mce-min3.txt --to-tsv

#To export taxonomy data

qiime tools export taxonomy_silva.qza --output-dir taxonomy

#To convert the taxonomy table from biom to txt

biom convert -i taxonomy/taxonomy.biom -o taxonomy/export-taxonomy.txt --to-tsv
